...
function cargarPreguntas() {
    return fetch(SUPABASE_URL + '/rest/v1/preguntas?select=data&id=eq.1', {
        headers: SUPABASE_HEADERS
    })
        .then(res => {
...
function guardarPreguntas() {
    localStorage.setItem('campeonisimo_preguntas', JSON.stringify(preguntas));
    fetch(SUPABASE_URL + '/rest/v1/preguntas?id=eq.1', {
        method: 'PATCH',
        headers: SUPABASE_HEADERS,
        body: JSON.stringify({ data: preguntas })
    })
        .then(res => {
            if (!res.ok) throw new Error('HTTP ' + res.status);
        ...
        .catch(() => {
            mostrarToast('Guardado local (sin conexion)');
        });
}
...
function guardarEdicionPregunta(idx) {
    ...
    preguntas[editorAsignatura][idx] = { pregunta: texto, opciones, correcta };
    guardarPreguntas();
    renderizarPreguntas();
