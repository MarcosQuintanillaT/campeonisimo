...
function cargarPuntajes() {
    return fetch(SUPABASE_URL + '/rest/v1/puntajes?select=*&order=puntaje.desc&limit=10', {
        headers: SUPABASE_HEADERS
    })
        .then(res => {
            if (!res.ok) throw new Error();
        ...
        .then(rows => {
            localStorage.setItem('campeonisimo_puntajes', JSON.stringify(rows));
            return rows;
        })
        .catch(() => {
...
function guardarPuntaje(puntaje) {
    ...
    fetch(SUPABASE_URL + '/rest/v1/puntajes', {
        method: 'POST',
        headers: SUPABASE_HEADERS,
        body: JSON.stringify({ nombre, puntaje })
    }).catch(() => {});

    const puntajes = JSON.parse(localStorage.getItem('campeonisimo_puntajes') || '[]');
    ...
    cargarPuntajes().then(puntajes => {
        ...
        if (puntajes.length === 0) {
            ...
            lista.innerHTML = puntajes.map((p, i) => {
                const fecha = p.created_at ? new Date(p.created_at).toLocaleDateString('es-ES') : (p.fecha || '');
                return `
                <div class="puntaje-item">
                    ...
                    <span class="puntaje-fecha">${fecha}</span>
                </div>`;
            }).join('');
        }

        const botones = document.getElementById('botones-puntajes');
        botones.innerHTML = '<button class="btn btn-volver" onclick="mostrarPantalla(\'menu\')">← Volver al Menú</button>';
        if (rolActual === 'admin') {
            botones.innerHTML += '<button class="btn btn-reiniciar" onclick="confirmarReinicio()">🗑 Reiniciar Puntajes</button>';
        }
    });
}

function reiniciarPuntajes() {
    fetch(SUPABASE_URL + '/rest/v1/puntajes?id=gt.0', {
        method: 'DELETE',
        headers: SUPABASE_HEADERS
    })
        .then(() => {
        ...
        .catch(() => {
            ...
            cerrarModal();
            mostrarPuntajes();
            mostrarToast('Puntajes eliminados localmente');
        });
}
