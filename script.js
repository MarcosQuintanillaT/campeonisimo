...
// ====== ESTADO DEL JUEGO ======
const PREGUNTAS_JSON = 'preguntas.json';
let preguntas = {};
...
function cargarPreguntas() {
    return fetch(PREGUNTAS_JSON + '?t=' + Date.now(), { cache: 'no-store' })
        .then(res => {
            if (!res.ok) throw new Error('HTTP ' + res.status);
            return res.json();
        })
        .then(data => {
            if (data && (data.matematicas || data.espanol || data.sociales || data.naturales || data.cultura)) {
                preguntas = data;
...
function guardarPreguntas() {
    localStorage.setItem('campeonisimo_preguntas', JSON.stringify(preguntas));
    mostrarToast('Guardado local. Exporta para compartir.');
}
